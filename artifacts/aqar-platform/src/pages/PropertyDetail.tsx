...
@media (max-width: 1023px) {
  .pd-mobilebar { display: flex; gap: 10px; position: fixed; bottom: 0; left: 0; right: 0; z-index: 50; background: #fff; border-top: 1px solid #EEF1F5; padding: 10px 14px calc(10px + env(safe-area-inset-bottom)); box-shadow: 0 -6px 20px rgba(15,23,42,0.08); transition: transform .28s ease, opacity .28s ease; }
  /* Slide fully out of view (incl. shadow) once the footer is on screen */
  .pd-mobilebar--hidden { transform: translateY(130%); opacity: 0; pointer-events: none; }
...
export default function PropertyDetail() {
  ...
  const [footerVisible, setFooterVisible] = useState(false); // hide the mobile CTA bar over the footer

  // Slide the sticky mobile contact bar out of the way once the footer scrolls
  // into view, so it never overlaps the footer at the bottom of the page.
  // Direct check on scroll — one getBoundingClientRect + a setState that React
  // no-ops unless the boundary is actually crossed, so it stays cheap.
  useEffect(() => {
  ...
  return (
    <MainLayout>
      ...
      {property.office && (property.office.phone || property.office.whatsapp) && (
        <div className={`pd-mobilebar${footerVisible ? " pd-mobilebar--hidden" : ""}`} dir="rtl">
          {property.office.whatsapp && (
