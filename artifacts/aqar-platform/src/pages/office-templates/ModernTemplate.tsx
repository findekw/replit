...
export default function ModernTemplate(p: TemplateProps) {
  ...
  return (
    <div dir="rtl" className="tm-root">
      ...
      {/* Mobile sticky — position:sticky docks it above the footer at page end */}
      {(hasWA || hasPhone) && (
...
/* Sticky (not fixed): floats at the viewport bottom while scrolling, then rests
   just above the footer at page end — native, smooth, never overlaps it. */
.tm-sticky { position: sticky; bottom: 0; z-index: 40; display: flex; justify-content: center; gap: 12px; padding: 12px 16px; background: rgba(255,255,255,0.96); backdrop-filter: blur(10px); border-top: 1px solid #EEF1F5; box-shadow: 0 -4px 20px rgba(15,23,42,0.08); }
